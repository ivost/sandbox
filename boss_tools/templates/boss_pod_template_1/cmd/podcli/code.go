...
var codeInitCmd = &cobra.Command{
	Use:     `init [name]`,
	Short:   "clone pod template",
	Example: `mkdir my_pod && cd my_pod && podcli code init`,
	RunE:    runCodeInit,
}

func init() {
	codeInitCmd.Flags().StringVar(&codeVersion, "version", "master", "pod repo tag or branch name")
...
//todo
func runCodeInit(cmd *cobra.Command, args []string) (err error) {
	...
	//versionName := podName
	//downloadURL := "https://github.com/braincorp/boss_pod_template/raw/master/archive/master.zip"
	////downloadURL := fmt.Sprintf("https://github.com/braincorp/boss_pod_template/raw/master/archive/%s.zip", codeVersion)
	...
	to := podName
	dirs := []string{"pkg", "cmd", "ui", "kustomize", "docs"}
	...
	makeFiles := []string{"Makefile", "Dockerfile", "go.mod", "go.sum", "LICENSE", "README.md"}
