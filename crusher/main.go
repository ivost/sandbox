...
var (
	...
	ignoreTLS        = flag.Bool("i", true, "Ignore TLS/SSL certificate validation ")
	method           = flag.String("m", "GET", "HTTP Method (GET,POST,PUT,DELETE)")
	numberOfRequests = flag.Int("r", 100, "Number of requests (use instead of -t)")
	...
	url              = flag.String("u", "http://localhost:8080/sr", "URL to run against")
	wait             = flag.String("w", "0ms", "Fixed delay between requests (e.g. 100ms - if non-zero will trump normal distribution)")
	mean             = flag.Float64("e", 100., "Request delay μ mean (in ms)")
	deviation        = flag.Float64("d", 80., "Standard deviation (in ms)")
	verbose          = flag.Bool("v", false, "verbose - use only for sanity check")
)
...
func (baton *Baton) run() {
	...
	//fmt.Printf("wait %v -> delay %v\n", *wait, w)
	if err != nil {
		fmt.Printf("Invalid duration %s - set to 0ms\n", *wait)
		w = 0
	}
	μ := *mean
	dev := *deviation
	if w == 0 {
		fmt.Printf("mean: %v ms, stddev: %v ms\n", μ, dev)
	}
	...
	start := time.Now()
	for c := 1; c <= baton.configuration.concurrency; c++ {
		var worker workable
		if preparedRunConfiguration.timedMode {
			...
			worker = newCountWorker(baton.configuration, preparedRunConfiguration.requests, preparedRunConfiguration.results, preparedRunConfiguration.done)
		}
		...
		if preparedRunConfiguration.preLoadedRequestsMode {
			...
			go worker.sendRequestWithDelay(request, w, μ, dev)
		}
	}
...
// var duration_Milliseconds time.Duration = n * time.Millisecond
// duration_Milliseconds.Nanoseconds() / 1e6
